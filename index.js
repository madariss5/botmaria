...
async function startMaria() {
  ...
  console.log(
    color(
      "\nHello, My Darling I'm Maria a whatsapp bot",
      "aqua"
    )
  );
  console.log(
    color(
      "\ncreated by Ayush pandey thanks for using me🍭",
      "aqua"
    )
  );
  console.log(color('\n📍YT CHANNEL: Ayush Botz ','silver'))
console.log(color('📍 GITHUB: Ayush-pandey-u ','silver'))
console.log(color('📍 WA NUMBER: +919931122319 ','silver'))
console.log(color('  📍Ayush Botz Inc. 2022📍','mediumseagreen'));
  const { state, saveCreds } = await useMultiFileAuthState("./Mystic Session");
  const Maria = MariaConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["Maria Bot", "Safari", "3.O"],
    auth: state,
  });
 ...
 Maria.ev.on('groups.update', async pea => {
     ...
     Maria.ev.on('groups.update', async pea => {
      ...
      let wm_fatih = { url: ppgc }
      if (pea[0].announce == true) {
        //Maria.send5ButImg(pea[0].id, `Grop has been *Closed!* Only *Admins* can send Messages!`, `Maria Bot`, wm_fatih, [])
  
        Maria.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Closed!* Only *Admins* can send Messages!' })
      } else if (pea[0].announce == false) {
        // Maria.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `Maria Bot`, wm_fatih, [])
        Maria.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Opened!* Now *Everyone* can send Messages!' })
      } else if (pea[0].restrict == true) {
        //Maria.send5ButImg(pea[0].id, `Group Info modification has been *Restricted*, Now only *Admins* can edit Group Info !`, `Maria Bot`, wm_fatih, [])
        ...
        //Maria.send5ButImg(pea[0].id, `Group Info modification has been *Un-Restricted*, Now only *Everyone* can edit Group Info !`, `Maria Bot`, wm_fatih, [])
  ...
  I am *Maria Bot*, Welcome to ${metadata.subject}.
